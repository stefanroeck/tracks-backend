...
class TcxGpxServiceTest {
    ...
    fun `parse sample tcx file`() {
        ...
        assertThat(result.activityWrapper?.activities).hasSize(1)
        with<Activity?, Any>(result.activityWrapper?.activities?.first()) {
            ...
            with(this?.laps?.first()) {
                ...
                assertThat(this?.trackpointsWrapper?.trackpoints).hasSize(794)

                with(this?.trackpointsWrapper?.trackpoints?.first()) {
